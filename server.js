...
const voterNames=()=>G.players.filter(p=>p.alive&&!p.cursed&&p.connected).map(p=>p.name);
...
const captain=()=>{
  let a=alive();
  // In test mode, keep the captaincy on real players so testers can always
  // drive room selection — bots never get a turn as captain.
  if(G.testMode){
    const real=a.filter(p=>!p.isBot);
    if(real.length)a=real;
  }
  return a[G.captainIdx%Math.max(a.length,1)];
};
...
function pubState(){
  ...
  return{
    ...
    players:G.players.map(p=>({name:p.name,color:p.color,alive:p.alive,connected:p.connected,cursed:!!p.cursed,
...
function action(type,data,fromPlayer,token=''){
  ...
  switch(type){
    ...
    case 'vote-eject':
      if(G.phase!=='extraction')return;
      if(!isAlivePlayer(fromPlayer)||G.players.find(p=>p.name===fromPlayer)?.cursed||!G.extraction.suspects.length)return;
  ...
  if(p==='/events'){
    ...
    if(type==='player'&&name){
      ...
      if(joinMode==='check'){
        ...
        G.players.push({name,color,role:null,alive:true,hacked:false,cursed:false,connected:true,token:newToken()});
        push();
...
<p style="margin-top:16px"><a href="/host">Host screen</a> · <a href="/">Player screen</a></p>
  ...
  if(p==='/map'){
    res.writeHead(200,{'Content-Type':'text/html'});
    res.end(fs.readFileSync(path.join(__dirname,'map.html')));return;
  }

  // ── Static assets (images, etc.) ─────────
  if(p.startsWith('/assets/')){
    const filePath=path.join(__dirname,'assets',p.slice(8));
    if(fs.existsSync(filePath)){
      const ext=path.extname(filePath).toLowerCase();
      const mime={'png':'image/png','jpg':'image/jpeg','jpeg':'image/jpeg','gif':'image/gif','webp':'image/webp'}[ext.slice(1)]||'application/octet-stream';
